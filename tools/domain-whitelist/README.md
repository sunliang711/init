...
`domain-whitelist` 用于在 Linux 上按域名和静态 IP 维护整机入站来源白名单。它会定时解析域名，将域名解析结果和静态 IP 写入 `nftables` set 或 `iptables + ipset`，并丢弃非白名单来源的新入站流量。
...
```bash
sudo domain-whitelist add office.example.com home.example.net 203.0.113.10 2001:db8::10
...
`whitelist.allow` 一行一个域名、IPv4 或 IPv6，支持 `#` 注释。

## 注意事项

- 域名条目最终仍然会转换成 IP 白名单，CDN 或共享 IP 场景可能扩大允许范围。
