...
TOOL_ENTRY="/usr/local/bin/nomad-manager"
JOB_ENTRY="/usr/local/bin/nomad-job"
LEGACY_TOOL_ENTRY="/usr/local/sbin/nomad-manager"
LEGACY_JOB_ENTRY="/usr/local/sbin/nomad-job"
...
VAULT_JWT_PROFILE_DIR="${VAULT_JWT_PROFILE_DIR:-${NOMAD_ROOT_DIR}/data/vault-jwt}"
BOOTSTRAP_ACL=1
TMPDIR_TO_CLEAN=""
AUDIT_ACTIVE=0
AUDIT_FINALIZED=0
...
Uninstall options:
  --remove-tools          Also remove ${TOOL_ENTRY}, ${JOB_ENTRY}, ${TOOL_PATH}, ${JOB_PATH} and ${TOOL_DIR}.
...
Runtime layout:
  ${NOMAD_ROOT_DIR} is the Nomad root directory managed by this script.
  ${BIN_PATH} stores the Nomad binary. ${BIN_ENTRY} is a symlink to it.
  ${CONFIG_DIR} stores Nomad configuration managed by this script.
  ${DATA_DIR} stores Nomad runtime data. Nomad agent data_dir is ${NOMAD_AGENT_DATA_DIR}.
  ${DATA_POINTER_FILE} links the data directory back to the installed tool metadata.

Installed tool snapshot:
  ${TOOL_DIR}
  ${TOOL_PATH} -> ${TOOL_DIR}/manager.sh
  ${JOB_PATH} -> ${TOOL_DIR}/job
...
4. Check service and cluster state:
   systemctl status nomad
   nomad server members
   nomad node status

5. Check installed management snapshot and audit log:
   ${TOOL_ENTRY} help
   cat ${INSTALL_METADATA_FILE}
   tail -n 20 ${AUDIT_LOG_FILE}

Notes:
  The install command writes default managed telemetry and Docker config files.
  These files are script-managed defaults, not files generated by Nomad itself.
  Config files are stored in ${CONFIG_DIR}.
  Runtime data is stored in ${DATA_DIR}; the Nomad agent data_dir is ${NOMAD_AGENT_DATA_DIR}.
EOF
      ;;
    docker)
      cat <<EOF
...
Notes:
  ...
  docker disable-driver changes the Nomad client driver denylist.
EOF
      ;;
    vault)
      cat <<EOF
...
Notes:
  ...
  Vault auth methods, policies and roles still need to exist in Vault.
EOF
      ;;
    vault-jwt)
      cat <<EOF
...
Notes:
  apply writes Nomad Vault config, enables or reuses Vault JWT Auth, writes policy and role,
...
Notes:
  ...
  ui reset removes the managed UI config. Nomad defaults ui.enabled to true, so reset can make UI available again.
EOF
      ;;
    job)
      cat <<EOF
Scenario: create and run a Nomad job

1. Generate a Docker job HCL:
...
EOF
    ...
    troubleshoot)
      cat <<EOF
Scenario: Nomad troubleshooting
...
3. Check cluster and client state:
   nomad server members
   nomad node status
   nomad node status -self
...
safe_remove_path() {
  ...
  case "$path" in
    "" | "/" | "/usr" | "/usr/local" | "/usr/local/bin" | "/etc" | "/opt" | "/var" | "/var/lib")
...
install_tool_snapshot() {
  ...
  if [ -f "${TOOL_DIR}/job" ]; then
    run_root ln -sfn "${TOOL_DIR}/job" "$JOB_PATH"
    run_root ln -sfn "$JOB_PATH" "$JOB_ENTRY"
  fi
  if [ -L "$LEGACY_TOOL_ENTRY" ]; then
    safe_remove_path "$LEGACY_TOOL_ENTRY"
  fi
  if [ -L "$LEGACY_JOB_ENTRY" ]; then
    safe_remove_path "$LEGACY_JOB_ENTRY"
  fi

  log_info "Nomad manager path installed: ${TOOL_PATH}"
  log_info "Nomad manager entry installed: ${TOOL_ENTRY}"
  if [ -f "${TOOL_DIR}/job" ]; then
    log_info "Nomad job path installed: ${JOB_PATH}"
...
ensure_nomad_user() {
  ...
  run_root useradd --system --home "$NOMAD_ROOT_DIR" --shell /bin/false "$NOMAD_USER"
}

install_directories() {
  log_info "Creating Nomad directories"
  run_root install -d -m 0755 -o root -g root "$NOMAD_ROOT_DIR"
  run_root install -d -m 0755 -o root -g root "$BIN_DIR"
  run_root install -d -m 0755 -o root -g root "${NOMAD_ROOT_DIR}/etc"
  run_root install -d -m 0755 -o root -g root "${NOMAD_ROOT_DIR}/data"
  run_root install -d -m 0755 -o root -g root "${NOMAD_ROOT_DIR}/lib"
  run_root install -d -m 0750 -o root -g root "${NOMAD_ROOT_DIR}/log"
  ...
  run_root install -d -m 0755 -o "$NOMAD_USER" -g "$NOMAD_GROUP" "$NOMAD_AGENT_DATA_DIR"
}
...
install_binary() {
  ...
  log_info "Nomad binary entry installed: ${BIN_ENTRY}"
  "$BIN_PATH" version
}
