...
def color_enabled() -> bool:
    value = os.environ.get("MONCTL_COLOR", "auto").lower()
...
def fetch_url(url: str, *, timeout: int = 60, no_proxy: bool = False) -> bytes:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "monctl/1"})
...
def extract_tar_gz(tar_file: str | Path, output_dir: str | Path) -> None:
    """Extract a release tarball, refusing members that would land outside output_dir.

    Python 3.12 gained extractall(filter="data"), but these tools also run on
    older interpreters, where extractall happily follows an absolute path or a
    "../" prefix in the archive. The members are checked here instead, so the
    behaviour does not depend on the interpreter the node happens to ship.
    """
        ...
        for member in members:
            if member.issym() or member.islnk():
                ...
                if link.is_absolute() or ".." in link.parts:
                    raise CLIError(f"Refuse to extract link outside the target directory: {member.name}")
                continue
            if not (member.isfile() or member.isdir()):
                raise CLIError(f"Refuse to extract unsupported archive member: {member.name}")
            target = (destination / member.name).resolve()
            if target != destination and destination not in target.parents:
                raise CLIError(f"Refuse to extract outside the target directory: {member.name}")
