...
install(){
    interface=${1:?'missing network interface,eg: eth0'}
    _require_root
    _require_linux
    _require_command ifconfig

    scriptFile="/usr/local/bin/traffic.sh"
    output="/tmp/traffic.output"

    echo -n "-- check interface.."
    if ! ifconfig "${interface}" >/dev/null 2>&1;then
        echo " not found interface: ${interface}"
        exit 1
    fi
    printf "\n"

    echo -n "-- generate script file: ${scriptFile}"
    cat<<EOF>"${scriptFile}"
#!/bin/bash
...
EOF
    ...
    rotateConfig="/etc/logrotate.d/traffic"
    echo -n "-- install logrotate config to ${rotateConfig}.."
    cat<<EOF>"${rotateConfig}"
${output}
{
    rotate 2
    daily
