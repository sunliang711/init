...
update() {
    ...
    log "INFO" "Try to get new public ip.."
    pub_ip="$(curl -s ipv6.ip.sb)"
    if [ -z "${pub_ip}" ];then
        log "error" "can not get ip"
        exit 1
    fi
    log "INFO" "New pub ipv6: ${pub_ip}"

    full_domain="${rr}.${domain}"

    log "INFO" "Get ${full_domain} current ip.."
    current_ip="$(nslookup ${full_domain} | grep 'Address:' | tail -n 1 | awk '{print $2}')"
    if [ -z "${current_ip}" ];then
        log "error" "can not get current ip of domain: ${full_domain}"
        exit 1
    fi
    log "INFO" "Current ip of domain is: ${current_ip}"

    if [ "${pub_ip}" == "${current_ip}" ];then
        log "INFO" "Current ip equals to new ip, skip"
        exit 0
    fi

    log "INFO" "Update new ip: ${pub_ip} to domain: ${full_domain}"
    curl "${api}" -d "{\"new_ip\":\"${pub_ip}\",\"rr\":\"${rr}\",\"domain\":\"${domain}\",\"type\":\"AAAA\"}"
