...
`target remove` / `job remove` 只停止采集,已经写进 TSDB 的序列留到 retention 到期。
这通常正是想要的,但也是「机器早就下线了,Grafana 下拉框里还列着」的来源。

`prometheus series delete --match '{job="x"}'` 是主动清理的路子:走 Prometheus 的 admin API
删序列,再 `clean_tombstones`。三个刻意的设计:

- **admin API 默认关**(`install --admin-api` 打开,`--no-admin-api` 关上)。它没有认证、
  能删光整个库,所以不该常开。doctor 在「开着 **且** 监听地址不是本地」时报 WARN。
- **先数后删**:打印每个选择器命中多少条序列,`--dry-run` 只看不动,默认要交互确认。
- **还在采的选择器直接拒绝**,要 `--force`。一个写错的选择器不该能删掉正在用的数据。

真机验证的结果里有一条必须写进帮助文本:**样本立刻且持久地消失**(重启后依然查不到),
**但索引条目不会马上消失** —— label 接口是从 block 索引回答的,一条索引要等它所在的 block
被压实掉才没。最近两小时内写的数据还在 head 里,于是会出现「图上没数据了、下拉框里名字还在」
的中间状态。不把这句说出来,跑完命令的人只会以为没生效。
