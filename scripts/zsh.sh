...
install() {
    ...
    _ensure_symlink "${ZSHRC_LINK_TARGET}" "${home}/.zshrc" || {
        echo "Please fork the repo first"
        exit 1
    }

    _ensureDir "${ZSH_CUSTOM}/themes" "${ZSH_CUSTOM}/plugins" "${home}/.ssh"
    ...
    # soft link sshconfig
    _ensure_symlink "${SSH_CONFIG_LINK_TARGET}" "${home}/.ssh/config"

    _write_state "${managed_autosuggestions_dir}" "${managed_syntax_highlighting_dir}"
}

linksshpems() {
    _ensure_symlink "${this}/../softlinks/sshpems" "${home}/.ssh/sshpems"
